function ControlScene(viewer){
    ...
    window.myArrPolygon = myArrPolygon
    ...
    window.carNumArray = carNumArray
    var entitiesActually = viewer.entities;//区域管理专用变量
    window.entitiesActually = entitiesActually;
    ...
    viewer.screenSpaceEventHandler.setInputAction(function onLeftClick(movement) {
        ...
        if(pickedObject instanceof Cesium.Cesium3DTileFeature)  //pick到3dtiles
        {
            ...
            if(featureName == "_01_1082NS0020013SP0052X110901_OID_3418_Object03_FACES_Model01" && clickStatus1==0)
            {
                ...
                //117.0316145109,36.6799150559

                // var x1=117.03169; //117.0316181401,36.6799126616
                // var x2=117.03166;
                ...
                }//117.0316174764,36.6799123078,31
               ...
               var myArra =
                ...
                clickStatus2=1;
                // viewer.camera.flyTo({
                //     destination : Cesium.Cartesian3.fromDegrees(117.0320059051,36.6794417967,90),
                //     orientation : {
            ...
            var createGridDiv =function(num){
                ...
                //     var backlogId =Number(backlogIdString.replace(/[^0-9]/ig,""))
                //     num = backlogId
                //     createGridDiv(num)
    
                //     // createGridDiv=function(num){
                //     //     smartReminder.shopEventClick()
                //     //     getShopInformation(num)
                //     // }
            ...
            if(pickedObject.id._id.match(RegExp('myentity'))){ //点击到商铺管理网格  shopGrid 
                getWhichJson = 'myentity';             
                var linShiId=pickedObject.id._id
                var gridBoxId =Number(linShiId.replace(/[^0-9]/ig,""))//获取网格id 进行匹配后台数据  提取字符串中的数字
            ...
            //商铺网格存在闪烁
            if(pickedObject.id._id.match("blinkEntity")){ 
                ...
                var shopEventUl='shopEvent'+backlogIdNumber
                var ulId='#'+shopEventUl

                backlogId ='backlog'+backlogIdNumber
                var blinkEntityHierarchy =entitiesActually.getById(pickedObject.id._id).polygon.hierarchy._value
                if(backlogIdNumber == 0){
                    clearInterval(shopBlink0)
                }else{
                    clearInterval(shopBlink1)
                }
                //clearInterval(shopBlink1)
                entitiesActually.removeById(pickedObject.id._id)
                ...
                })//添加hightLightEntity表示选中
                entitiesActually.add({
                    id:backlogId,
                    polygon:{
                    hierarchy : Cesium.Cartesian3.fromDegreesArrayHeights(myArrPolygon[backlogIdNumber]),                         
                    height:27.001,//定义了高度就不会贴着osgb了
                    //颜色回调
                ...
                createGridDiv(num)
                setTimeout(function(){
                    if($('.roadEventGrid').length == 0 && $('.shopEventGrid').length ==1){
                        $('#messageRemind').removeClass('currentMessages').addClass('defaultMessages')
                    }
                    $(ulId).remove() //临时这么写 这里可能会引起事件列表为空时 工具栏消息提醒图标未变白2删除不掉其他网格对应的消息列表内的ul
                },1000)
            ...
            if(pickedObject.id._id.match(RegExp('roadBacklog'))){
                $('.roadUl').css('display','block')
            }
            //最初的弹出店铺面板  
